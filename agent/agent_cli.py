...
class LicenseAgentCLI:
    ...
    def print_banner(self):
        ...
        print(f"{Fore.CYAN}🔐 ZAYONA LICENSE MANAGEMENT AGENT")
        print(f"{Fore.CYAN}{'='*60}")
        print(f"{Fore.WHITE}Version: 1.0.0")
        print(f"{Fore.WHITE}API Server: {config.API_BASE_URL}")
    ...
    def print_menu(self):
        ...
        print(f"{Fore.WHITE}6. {Fore.RED}Exit{Style.RESET_ALL}")
    ...
    def test_api_connection(self):
        ...
        try:
            print(f"{Fore.YELLOW}🔄 Testing connection to: {config.API_BASE_URL}{Style.RESET_ALL}")
            
            if self.api_client.test_connection():
                print(f"{Fore.GREEN}✅ API connection successful!{Style.RESET_ALL}")
                
                # Get health info
                health_info = self.api_client.health_check()
                print(f"{Fore.WHITE}Server Status: {Fore.GREEN}{health_info.get('status', 'Unknown')}{Style.RESET_ALL}")
                print(f"{Fore.WHITE}Database: {Fore.GREEN}{health_info.get('database', 'Unknown')}{Style.RESET_ALL}")
            else:
    ...
    def show_hardware_fingerprint(self):
        ...
        try:
            ...
            # Show components
            components = self.hardware_fingerprint.get_fingerprint_components()
            print(f"\n{Fore.WHITE}Components:{Style.RESET_ALL}")
            for key, value in components.items():
                if key != 'full_fingerprint':
    ...
    def run(self):
        ...
        while True:
            try:
                self.print_menu()
                choice = self.get_user_input("Enter your choice (1-6)", required=True)
