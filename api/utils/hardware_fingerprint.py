"""
Hardware fingerprinting utilities for license anti-sharing protection
"""
...
class HardwareFingerprint:
        ...
        try:
            if self.system == "windows":
                ...
                if result.returncode == 0:
                    lines = result.stdout.strip().split('\n')
                    if len(lines) > 1:
                        bios_info['serial'] = lines[1].strip()
            
            elif self.system == "linux":
                # Linux: read from /sys/class/dmi/id
                try:
                    with open('/sys/class/dmi/id/board_serial', 'r') as f:
                        bios_info['serial'] = f.read().strip()
                except FileNotFoundError:
                    pass
                ...
                result = subprocess.run(['system_profiler', 'SPHardwareDataType'], capture_output=True, text=True)
                if result.returncode == 0:
                    serial_match = re.search(r'Serial Number \(system\):\s+(.+)', result.stdout)
                    if serial_match:
                        bios_info['serial'] = serial_match.group(1).strip()
        
        except Exception as e:
            print(f"Warning: Could not get BIOS info: {e}")
    ...
    def generate_fingerprint(self) -> str:
        ...
        if bios_info.get('serial'):
            fingerprint_parts.append(f"BIOS:{bios_info['serial']}")
    ...
    def get_fingerprint_components(self) -> Dict[str, str]:
        """Get individual fingerprint components"""
        return {
            'mac_addresses': ','.join(self.get_mac_addresses()),
            'cpu_info': str(self.get_cpu_info()),
            'disk_info': str(self.get_disk_info()),
            'system_uuid': self.get_system_uuid() or 'Unknown',
            'bios_info': str(self.get_bios_info()),
            'full_fingerprint': self.generate_fingerprint()
        }
    ...
    def validate_fingerprint(self, stored_fingerprint: str, current_fingerprint: str) -> bool:
        ...
        # or allow some components to change (like MAC addresses)
